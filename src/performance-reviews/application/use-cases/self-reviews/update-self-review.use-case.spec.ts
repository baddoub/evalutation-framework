...
describe('UpdateSelfReviewUseCase', () => {
  ...
  const createValidReviewCycle = (): ReviewCycle => {
    const deadlines = CycleDeadlines.create({
      selfReview: new Date(Date.now() + 86400000), // 1 day in future
      peerFeedback: new Date(Date.now() + 172800000), // 2 days in future
      managerEvaluation: new Date(Date.now() + 259200000), // 3 days in future
      calibration: new Date(Date.now() + 345600000), // 4 days in future
      feedbackDelivery: new Date(Date.now() + 432000000), // 5 days in future
    })
  ...
  beforeEach(() => {
    ...
    useCase = new UpdateSelfReviewUseCase(mockSelfReviewRepository, mockCycleRepository)
  })
  ...
  describe('CRITICAL: Cycle Validation', () => {
    ...
    it('should throw error when self-review deadline has passed', async () => {
      ...
      const passedDeadline = new Date(Date.now() - 86400000) // 1 day in past
      const deadlines = CycleDeadlines.create({
  ...
  describe('CRITICAL: Updating Narrative', () => {
    it('should update narrative when provided', async () => {
      ...
      const newNarrative = Narrative.create('This is my updated narrative text')
  ...
  describe('CRITICAL: Updating Both Scores and Narrative', () => {
    it('should update both scores and narrative when both provided', async () => {
      ...
      const newNarrative = Narrative.create('Updated narrative after both changes')
  ...
  describe('IMPORTANT: Repository Persistence', () => {
    ...
    it('should call save with the modified review entity', async () => {
      ...
      const newNarrative = Narrative.create('Persisted narrative')
  ...
  describe('EDGE: Updating Already Submitted Review', () => {
    ...
    it('should throw exception when trying to update narrative on submitted review', async () => {
      ...
      const newNarrative = Narrative.create('Trying to update submitted review')
    ...
    it('should throw exception when trying to update both on submitted review', async () => {
      ...
      const input = {
        ...
        narrative: Narrative.create('Updated narrative'),
      }
  ...
  describe('EDGE: Partial Updates', () => {
    it('should handle updating only scores without narrative', async () => {
      ...
      const cycleId = ReviewCycleId.generate()
      const cycle = createValidReviewCycle()
      const originalNarrative = Narrative.create('Original narrative')
    ...
    it('should handle updating only narrative without scores', async () => {
      ...
      const newNarrative = Narrative.create('Only narrative is updated')
    ...
    it('should maintain original values when partial update provided', async () => {
      ...
      const cycleId = ReviewCycleId.generate()
      const cycle = createValidReviewCycle()
      const originalNarrative = Narrative.create('Original narrative text')
      const review = createValidSelfReview({
        cycleId,
        userId,
        narrative: originalNarrative,
  ...
  describe('EDGE: Empty Update', () => {
    ...
    it('should not call updateScores when scores not provided', async () => {
      ...
      const input = {
        userId,
        cycleId,
        narrative: Narrative.create('Only narrative update'),
      }
  ...
  describe('Integration: Full Workflow', () => {
    it('should execute complete update workflow with all validations', async () => {
      ...
      const input = {
        ...
        narrative: Narrative.create('Comprehensive update with all validations'),
      }
